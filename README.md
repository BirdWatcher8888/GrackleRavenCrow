...
For the two **reserved** zones on each form, add per-row **dropdown questions** ("Row C — pick your seat") listing each sellable seat once. Zeffy retires a dropdown option once it's bought — that's the double-sell lock. Tier 2 / Balcony ticket types are quantity-only (set their capacity: 186 and 129, minus anything extra you're holding).
